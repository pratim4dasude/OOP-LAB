#include <iostream>
using namespace std;
struct student
{
    char name[30];
    int rollno;
    int marks[5];
    void input()
    {
        cout << "Enter student's name: ";
        cin >> name;
        cout << "Enter Roll Number of student : ";
        cin >> rollno;
        cout << "Enter the marks of 5 subject:" << endl;
        for (int i = 0; i < 5; ++i)
        {
            cin >> marks[i];
        }
    }
    int total()
    {
        int sum = 0;
        for (int i = 0; i < 5; ++i)
        {
            sum += marks[i];
        }
        return sum;
    }
};
void dispTotalNPercent(student s[], int n);
void dispStudByRoll(student s[], int n, int searchroll);
void dispStudByPercent(student s[], int n, int min, int max);
void sortArrAscTotal(student s[], int n);
int main()
{
    int n;
    cout << "Enter the number of students: ";
    cin >> n;
    student s[n];
    for (int i = 0; i < n; ++i)
    {
        cout << endl
             << "Student " << i + 1 << ":\t" << endl;
        s[i].input();
    }
    //calling the functions
    dispTotalNPercent(s, n);
    int roll;
    cout << "\n\nEnter the roll no. to search: ";
    cin >> roll;
    dispStudByRoll(s, n, roll);
    int max, min;
    cout << "\nEnter the max percentage: ";
    cin >> max;
    cout << "Enter the min percentage : ";
    cin >> min;
    dispStudByPercent(s, n, max, min);
    sortArrAscTotal(s, n);
}
void dispTotalNPercent(student s[], int n)
{
    int percent;
    for (int i = 0; i < n; ++i)
    {
        cout << endl
             << "Student " << i + 1 << ":\tStudent Name: " << s[i].name << "\tTotal marks : " << s[i].total() << "\tPercentage Obtain: " << ((float)s[i].total() / 500) * 100 << "%";
    }
}
void dispStudByRoll(student s[], int n, int searchroll)
{
    for (int i = 0; i < n; ++i)
    {
        if (s[i].rollno == searchroll)
        {
            cout << endl
                 << "Student " << i + 1 << ":\tStudent Name: " << s[i].name << "\tRoll Number: " << s[i].rollno << endl
                 << "Marks of student : \n";
            for (int j = 0; j < 5; ++j)
            {
                cout << s[i].marks[j] << endl;
            }
        }
    }
}
void dispStudByPercent(student s[], int n, int max, int min)
{
    for (int i = 0; i < n; ++i)
    {
        if (((float)s[i].total() / 500) * 100 <= max && ((float)s[i].total() / 500) * 100 >= min)
        {
            cout << endl
                 << "Student " << i + 1 << ":"
                 << "\tName of the student : " << s[i].name << "\tRoll Number of student : " << s[i].rollno << "\tPercentage Obtain: " << ((float)s[i].total() / 500) * 100 << "%";
        }
    }
}
void sortArrAscTotal(student s[], int n)
{
    cout << endl 
         << "\nAscending Order of Marks" << endl;
    int index[n];
    for (int i = 0; i < n; ++i)
    {
        index[i] = i;
    }
    for (int i = 0; i < n - 1; ++i)
    {
        for (int j = 0; j < n - 1; ++j)
        {
            if (s[index[j]].total() > s[index[j + 1]].total())
            {
                index[j] += index[j + 1]; //swapping
                index[j + 1] = index[j] - index[j + 1];
                index[j] -= index[j + 1];
            }
        }
    }
    for (int i = 0; i < n; ++i)
    {
        cout << endl
             << "Student " << index[i] + 1 << ":\tName of the student: " << s[index[i]].name << "\tRoll Number of the student : " << s[index[i]].rollno << "\tTotal Obtain: " << s[index[i]].total();
    }
}